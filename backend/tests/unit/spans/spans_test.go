...
func TestSpans(t *testing.T) {
	...
	testCases := []unit.TestCase{
		...
		},
		{
			...
			ReadCalledWith: []map[string]interface{}{
				{
					"str":        "SELECT Timestamp as timestamp, SpanId as spanId, ParentSpanId as parentSpanId, TraceId as traceId, ServiceName as serviceName, SpanName as name, SpanKind as kind, Duration as duration, arrayMap(key -> tuple(key, SpanAttributes[key]), SpanAttributes.keys) as tags FROM . WHERE timestamp>=? AND timestamp<=? AND SpanAttributes[?]=? ORDER BY timestamp DESC LIMIT 100 OFFSET 0",
		...
		},
		{
			...
			ReadCalledWith: []map[string]interface{}{
				{
					"str":        "SELECT Timestamp as timestamp, SpanId as spanId, ParentSpanId as parentSpanId, TraceId as traceId, ServiceName as serviceName, SpanName as name, SpanKind as kind, Duration as duration, arrayMap(key -> tuple(key, SpanAttributes[key]), SpanAttributes.keys) as tags FROM . WHERE timestamp>=? AND timestamp<=? AND SpanAttributes[?] ILIKE ? ORDER BY timestamp DESC LIMIT 100 OFFSET 0",
		...
		},
		{
			...
			ReadCalledWith: []map[string]interface{}{
				{
					"str":        "SELECT Timestamp as timestamp, SpanId as spanId, ParentSpanId as parentSpanId, TraceId as traceId, ServiceName as serviceName, SpanName as name, SpanKind as kind, Duration as duration, arrayMap(key -> tuple(key, SpanAttributes[key]), SpanAttributes.keys) as tags FROM . WHERE timestamp>=? AND timestamp<=? AND mapContains(SpanAttributes, ?) ORDER BY timestamp DESC LIMIT 100 OFFSET 0",
